## gamma mixture model (grouped data)

gammaEM2 <- function(x, ncomp = NULL, pi = NULL, mu = NULL, sd = NULL, ev, 
  ...
  if(length(pi) != length(mu) || length(pi) != length(sd) ) {
	...
	fit <- gamma_g_C(x, pi, alpha, lambda, method = mstep.method, max_iter, tol)
